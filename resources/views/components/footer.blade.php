<footer class="footer p-10" style="background-color: #2c3f50; color:#ffffff">
  <div class="max-w-screen-xl items-center justify-between mx-auto">
    <div class="grid grid-cols-3 gap-6">
      <div>
        <p>PT Ladang Usaha Jaya Bersama short company profile is here...</p>
      </div>
      <div>
        <h6 class="footer-title opacity-100">Contact</h6>
        <p>Phone : (021) 6530 1482</p>
        <p>Fax : (021) 6530 2313</p>
        <p>Email : LUJB_indo@hotmail.com</p>
        <h6 class="footer-title opacity-100 mt-4">Address</h6>
        <p>Jl. Gunung Sahari Raya 1,<br> Blok A-13 Jakarta Pusat 10720, Indonesia</p>
      </div>
      <div>
        <h6 class="footer-title opacity-100">Product</h6>
        <ul class="list-disc list-inside">
            <li class="">Sanki Fuel Pump</li>
            <li>Flexible Pipes</li>
            <li>Flowmeters</li>
            <li>Composite Hose</li>
            <li>Composite Panel</li>
            <li>Automatic Tank Gauge</li>
        </ul>
      </div>
    </div>
  </div>
</footer>

<div style="background-color: #2c3f50" class="border-t-2 border-gray-300" >
  <p class="text-center text-white p-3">Copyright LUJB 2024 All Rights Reserved.</p>
</div>