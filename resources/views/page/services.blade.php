<x-layout>
    ...
    <div class="hero h-auto" id="desktop">
        ...
        <p class="absolute w-80 px-4 py-6 text-white bg-[#229e89] rounded-2xl right-60 bottom-50 font-semibold text-4xl">{!! $datas[32]->value !!}</p>
    </div>

    <div class="hero h-auto" id="mobile">
        <img src= "{{ asset('storage/img/Banner/'. $banners[1]->image) }}" alt="Banner"/>
        <p class="absolute w-80 px-4 text-white top-60 bg-[#229e89] rounded-2xl font-semibold text-4xl">{!! $datas[32]->value !!}</p>
    </div>
        ...
        <div class="max-w-5xl mx-auto p-4">
            <div class="grid grid-cols-2 gap-10 items-center py-8 px-4 mx-auto max-w-screen-xl lg:grid lg:grid-cols-2 lg:py-16 lg:px-6">
                <div class="justify-items-center">
                    <img class="h-auto w-96 rounded-3xl shadow-lg" src="{{ asset('storage/img/GeneralImage/'. $images[0]->image) }}" alt="office content 1">
                </div>
                <div class="font-reguler text-gray-500 sm:text-lg dark:text-gray-400">
                    <h2 class="mb-2 text-3xl tracking-tight font-bold text-[#229e89] dark:text-white">{!! $datas[6]->value !!}</h2>
    ...
    <section class="bg-white dark:bg-gray-900" style="background-image: url('{{ asset('storage/img/GeneralImage/'. $images[3]->image) }}')" id="desktop">
        <div class="max-w-screen-xl mx-auto p-40">
            <h2 class="text-3xl tracking-tight font-bold text-[#229e89] dark:text-white text-center mb-2">{!! $datas[8]->value !!}</h2>
    ...
    <section id="desktop">
        <div class="max-w-screen-xl mx-auto p-4">
            <div class="grid grid-cols-2 gap-10 items-center py-8 px-4 mx-auto max-w-screen-xl lg:grid lg:grid-cols-2 lg:py-16 lg:px-6">
                    <div class="font-reguler text-gray-500 sm:text-lg dark:text-gray-400">
                        <h2 class="mb-2 text-3xl tracking-tight font-bold text-[#229e89] dark:text-white">{!! $datas[10]->value !!}</h2>
                        <p class="mb-4 text-black">{!! $datas[11]->value !!}</p>
                    </div>
                    <div class="justify-items-center">
                        <img class="w-96 rounded-3xl shadow-lg" src="{{ asset('storage/img/GeneralImage/'. $images[1]->image) }}" alt="equipment_install">
                    </div>
            </div>
        </div>
    </section>
    ...
    <section class="bg-white dark:bg-gray-900" id="desktop">
        <div class="max-w-screen-xl mx-auto p-4">
            <div class="grid grid-cols-2 gap-10 items-center py-8 px-4 mx-auto max-w-screen-xl lg:grid lg:grid-cols-2 lg:py-16 lg:px-6">
                <div class="justify-items-center">
                    <img class="w-full rounded-3xl shadow-lg" src="{{ asset('storage/img/GeneralImage/'. $images[2]->image) }}" alt="hydrostatic_test">
                </div>
                <div class="font-reguler text-gray-500 sm:text-lg dark:text-gray-400">
                    <h2 class="mb-2 text-3xl tracking-tight font-bold text-[#229e89] dark:text-white">{!! $datas[12]->value !!}</h2>
